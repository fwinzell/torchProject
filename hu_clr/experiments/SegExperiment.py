...
from hu_clr.datasets.two_dim.ProstateImageLoader import ProstateDataset, load_img_and_target_paths
from hu_clr.datasets.transformations import *
from torch.utils.data import DataLoader

from trixi.experiment.pytorchexperiment import PytorchExperiment

from hu_clr.networks.RecursiveUNet import UNet
from hu_clr.networks.unet_con import SupConUnet

from hu_clr.loss_functions.dice_loss import SoftDiceLoss

from hu_clr.loss_functions.metrics import dice_pytorch, SegmentationMetric
...
class SegExperiment(PytorchExperiment):
    ...
    def setup(self):
        tr_img_paths, tr_tar_paths, val_img_paths, val_tar_paths = load_img_and_target_paths(self.config.data_dir, val_split=self.config.val_split)
        # Define transformations
        transform = ComposeDouble([
            FunctionWrapperDouble(normalize_01),
            FunctionWrapperDouble(np.moveaxis, input=True, target=False, source=-1, destination=0)
        ])
        tr_dataset = ProstateDataset(tr_img_paths, target_paths=tr_tar_paths, mode='seg', transform=transform)
        self.train_data_loader = DataLoader(tr_dataset, batch_size=self.config.batch_size, shuffle=True, drop_last=True)
        val_dataset = ProstateDataset(val_img_paths, target_paths=val_tar_paths, mode='seg', transform=transform)
        self.val_data_loader = DataLoader(val_dataset, batch_size=self.config.batch_size, shuffle=True, drop_last=True)
        # Test loader?

        # self.model = UNet(num_classes=self.config.num_classes, num_downs=4)
        self.model = SupConUnet(num_classes=self.config.num_classes, in_channels=3)
        ...
        # self.writer = tb_logger.Logger(logdir=self.elog.work_dir, flush_secs=2)
    ...
    def train(self, epoch):
        ...
        for data_batch in self.train_data_loader:
            ...
            data = data_batch[0].float().to(self.device)
            target = data_batch[1].long().to(self.device)
