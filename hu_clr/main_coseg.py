...
def parse_option():
    ...
    parser.add_argument('--pretrained_model_path', type=str, default='/home/fi5666wi/Documents/Python/saved_models/hu_clr/b_10_model.pth',
    ...
    parser.add_argument('--train_sample', type=float, default=0.0, help='parameter for sampling rate of training set')
    ...
    if opt.data_folder is None:
        ...
        opt.data_folder = os.path.join('/home/fi5666wi/Documents/Prostate images', opt.data_folder)

    if opt.split_dir is None:
        opt.split_dir = os.path.join('./data', opt.dataset)
    opt.model_path = '/home/fi5666wi/Documents/Python/saved_models/hu_clr/{}_models'.format(opt.dataset)
    opt.tb_path = '/home/fi5666wi/Documents/Python/saved_models/hu_clr/{}_tensorboard'.format(opt.dataset)
...
def set_loader(opt):
    ...
    # tr_paths = load_img_paths(opt.data_folder, val_split=0.0)
    img_paths, target_paths = load_img_and_target_paths(opt.data_folder, val_split=0.0)
    # Define transformations
    transform = ComposeDouble([
        FunctionWrapperDouble(normalize_01),
        FunctionWrapperDouble(np.moveaxis, input=True, target=False, source=-1, destination=0)
    ])
    tr_dataset = ProstateDataset(img_paths, target_paths=target_paths, mode='sup', transform=transform)
    train_loader = DataLoader(tr_dataset, batch_size=opt.batch_size, shuffle=True, drop_last=True)
...
def set_model(opt):
    ...
    # model = LocalConUnet3(num_classes=128, in_channels=3)
...
def memory_check(msg=None):
    alloc = torch.cuda.memory_allocated()
    reser = torch.cuda.memory_reserved()
    total = torch.cuda.get_device_properties(torch.cuda.current_device()).total_memory

    print(msg + ':')
    print('Allocated: {} GiB'.format(alloc/1e9))
    print('Reserved: {} GiB'.format(reser/1e9))
    print('Total: {} GiB'.format(total/1e9))
