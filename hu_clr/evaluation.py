...
parser.add_argument("--saved_model_path", type=str,
                    default='/home/fi5666wi/Documents/Python/saved_models/hu_clr/20220630-103838_Unet_hippo'
...
def predict(img,
    ...
    with torch.no_grad():
        y, out = model(x)  # send through model/network
...
if __name__ == '__main__':
    ...
    state_dict = torch.load(config.saved_model_path)['model']
    ...
    overlap = 0
    img_ds = image_to_patches(prostate_image, overlap=overlap)
    gt_ds = image_to_patches(gt_image, overlap=overlap)
    ...
    res = patches_to_image(np.array(output), prostate_image.shape, overlap=overlap)
