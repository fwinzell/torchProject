...
class ChLocalLoss(nn.Module):
    def __init__(self,
        ...
        self.img_size_x, self.img_size_y = config.input_shape[-2:]
    ...
    def _define_local_regions(self):
        ...
        if self.no_of_local_regions == 9:
            ...
            pos_sample_indexes = np.zeros((self.no_of_local_regions, 2), dtype=np.int32)
            pos_sample_indexes[0], pos_sample_indexes[1], pos_sample_indexes[2] = [0, 0], [0, int(im_y / 2)], [0, im_y]
            pos_sample_indexes[3], pos_sample_indexes[4], pos_sample_indexes[5] = [int(im_x / 2), 0], \
                                                                                  [int(im_x / 2), int(im_y / 2)], \
                                                                                  [int(im_x / 2), im_y]
            pos_sample_indexes[6], pos_sample_indexes[7], pos_sample_indexes[8] = [im_x, 0], [im_x, int(im_y / 2)], \
                                                                                  [im_x, im_y]
            pos_sample_indexes[9], pos_sample_indexes[10] = [int(im_x / 4), int(im_y / 4)], \
                                                            [int(im_x / 4), int(3 * im_y / 4)]
            pos_sample_indexes[11], pos_sample_indexes[12] = [int(3 * im_x / 4), int(im_y / 4)], \
    ...
    def forward(self, y_1, y_2):
        ...
        for pos_index in range(0, self.bs, 2):
            ...
            x_num_i1 = y_1[num_i1]  # tf.gather(y_fin, num_i1)
            x_num_i2 = y_2[num_i1]  # tf.gather(y_fin, num_i2)
            ...
            if self.local_reg_size == 1:
                ...
                for local_pos_index in range(0, self.no_of_local_regions, 1):
                    ...
                    tmp_idx_0 = torch.cuda.LongTensor([self.pos_sample_indexes[local_pos_index, 0],
                                                       self.pos_sample_indexes[local_pos_index, 0] + 1,
                                                       self.pos_sample_indexes[local_pos_index, 0] + 2])
                    tmp_idx_1 = torch.cuda.LongTensor([self.pos_sample_indexes[local_pos_index, 1],
                                                       self.pos_sample_indexes[local_pos_index, 1] + 1,
                                                       self.pos_sample_indexes[local_pos_index, 1] + 2])
                    x_num_tmp_i1 = torch.index_select(x_num_i1, dim=2, index=tmp_idx_0)
                    x_num_tmp_i1 = torch.index_select(x_num_tmp_i1, dim=3, index=tmp_idx_1)
                    ...
                    # contributes to the numerator term. Fetch x and y coordinates
                    x_num_tmp_i2 = torch.index_select(x_num_i2, dim=2, index=tmp_idx_0)
                    x_num_tmp_i2 = torch.index_select(x_num_tmp_i2, dim=3, index=tmp_idx_1)
                    ...
                    for local_neg_index in range(0, no_of_neg_pts, 1):
                        ...
                        neg_tmp_idx_0 = torch.cuda.LongTensor([neg_samples_index_list[local_neg_index, 0],
                                                               neg_samples_index_list[local_neg_index, 0] + 1,
                                                               neg_samples_index_list[local_neg_index, 0] + 2])
                        neg_tmp_idx_1 = torch.cuda.LongTensor([neg_samples_index_list[local_neg_index, 1],
                                                               neg_samples_index_list[local_neg_index, 1] + 1,
                                                               neg_samples_index_list[local_neg_index, 1] + 2])

                        x_den_tmp_i1 = torch.index_select(x_num_i1, dim=2, index=neg_tmp_idx_0)
                        x_den_tmp_i1 = torch.index_select(x_den_tmp_i1, dim=3, index=neg_tmp_idx_1)
                        ...
                        x_den_tmp_i2 = torch.index_select(x_num_i2, dim=2, index=neg_tmp_idx_0)
                        x_den_tmp_i2 = torch.index_select(x_den_tmp_i2, dim=3, index=neg_tmp_idx_1)
            ...
            # if local region size is 1x1
                ...
                for local_pos_index in range(0, self.no_of_local_regions, 1):
                    ...
                    x_num_tmp_i1 = torch.index_select(x_num_i1, dim=2,
                                                      index=torch.cuda.LongTensor(
                                                          self.pos_sample_indexes[local_pos_index, 0]))
                    x_num_tmp_i1 = torch.index_select(x_num_tmp_i1, dim=3,
                                                      index=torch.cuda.LongTensor(
                                                          self.pos_sample_indexes[local_pos_index, 1]))

                    # corresponding positive local region 'pos_index_den' in feature map from image x_a2_i
                    # fetch x and y coordinates
                    x_num_tmp_i2 = torch.index_select(x_num_i2, dim=2,
                                                      index=torch.cuda.LongTensor(
                                                          self.pos_sample_indexes[local_pos_index, 0]))
                    x_num_tmp_i2 = torch.index_select(x_num_tmp_i2, dim=3,
                                                      index=torch.cuda.LongTensor(
                                                          self.pos_sample_indexes[local_pos_index, 1]))
                    ...
                    for local_neg_index in range(0, no_of_neg_pts, 1):
                        # negative local regions in feature map (f_a1_i) from image (x_a1_i)
                        x_den_tmp_i1 = torch.index_select(x_num_i1, dim=2,
                                                          index=torch.cuda.LongTensor(
                                                              neg_samples_index_list[local_neg_index, 0]))
                        x_den_tmp_i1 = torch.index_select(x_den_tmp_i1, dim=3,
                                                          index=torch.cuda.LongTensor(
                                                              neg_samples_index_list[local_neg_index, 1]))

                        # negative local regions in feature map (f_a2_i) from image (x_a2_i)
                        x_den_tmp_i2 = torch.index_select(x_num_i2, dim=2,
                                                          index=torch.cuda.LongTensor(
                                                              neg_samples_index_list[local_neg_index, 0]))
                        x_den_tmp_i2 = torch.index_select(x_den_tmp_i2, dim=3,
                                                          index=torch.cuda.LongTensor(
                                                              neg_samples_index_list[local_neg_index, 1]))
